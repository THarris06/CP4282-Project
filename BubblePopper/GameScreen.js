...
export default function GameScreen() {
  ...
  const fireLaser = () => {
    ...
    const originX = gunCenterX;
    const originY = screenHeight - 40;
    const angleRad = (gunAngle * Math.PI) / 180;

    // Check for hits immediately
    // checkHits(gunCenterX);
    checkHits(originX, originY, angleRad);
  ...
  const checkHits = (originX, originY, angleRad) => {
    ...
    const laserDirY = Math.sin(angleRad);
    ...
    setBubbles(prevBubbles => {
      ...
      prevBubbles.forEach(bubble => {
        ...
        if (dot > 0) {
          // Closest point on the laser to the bubble center
          const closestX = originX + laserDirX * dot;
          const closestY = originY + laserDirY * dot;

          // Distance from bubble center to laser line
          const dist = Math.hypot(
            closestX - bubbleCenterX,
            closestY - bubbleCenterY
          );
        
          // If laser is within bubble radius, it's a hit
          if (dist <= bubble.radius +4) {
